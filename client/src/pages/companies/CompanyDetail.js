...
import { getCompany } from "../../redux/apiCalls";
...
function CompanyDetail(props) {
  ...
  const { isFetching, err } = useSelector((state) => state.company);
  ...
  useEffect(() => {
    ...
    getCompany(dispatch,cid);
    // axios
    //   .get(`http://localhost:8081/api/company/${slug}`)
    //   .then((response) => {
  ...
  const handleClick = () => {
    window.location.href =
      "https://api.whatsapp.com/send?phone=918824885175&text=Hi%20we%20need%20help%20regarding%20something";
  };
  const handleMenuItemChange = (item) => {
    setstate({ ...state, activeMenuItem: item });
  };
  ...
  if (error) {
    return <Navigate to="/companies" exact />;
  }
  ...
  return (
    <div id="company-detail-container">
      ...
      {/* Modal form for buying and selling */}
              ...
              <form className="RequestQuote" onSubmit={submitDetails}>
                ...
                <div className="div_info info_category">
                  ...
                  >
                    ...
                    <option value="Investment Advisory Firms">
      ...
      <div id="company-detail-content">
        <div id="company-detail-header">
          <div id="company-name-header-left">
            <h1 id="company-header-name">{currentCompany.CompanyName}</h1>
            <div id="company-header-category">{currentCompany.Category}</div>
          ...
          <div id="company-name-header-right"> 
            <img
              ...
              src={`http://localhost:8081/static/${currentCompany.CompanyLogo?.substring(8)}`}
              style={{
                height: "200px",
                width: "200px",
                objectFit: "contain",
        ...
        <div>
          <div id="price" style={{ padding: "20px" }}>
            <Price company={state.company} id={state.id} />
          </div>
          <div id="about" style={{ padding: "20px" }}>
            <About company={state.company} id={state.id} />
          </div>
          <div id="financials" style={{ padding: "20px" }}>
            <Financials company={state.company} id={state.id} />
          </div>
          <div id="shareholding" style={{ padding: "20px" }}>
            <Shareholdings company={state.company} id={state.id} />
          </div>
          <div id="balancesheet" style={{ padding: "20px", width: "1000px" }}>
            <Balancesheet
              balancesheets={state?.company?.Balancesheets}
              id={state.id}
            />
          </div>
          <div id="ProfitAndLoss" style={{ padding: "20px", width: "1000px" }}>
            <ProfitAndLoss
              profitloss={state?.company?.ProfitAndLoss}
              id={state.id}
            />
          </div>
          <div id="AnnualReport" style={{ padding: "20px", width: "1000px" }}>
            <AnnualReport pdfs={state?.company?.AnnualReports} id={state.id} />
          </div>
        </div>
